...
public class DoubleStrike extends BaseCard {
    public static final String ID = makeID("Double Strike"); //makeID adds the mod ID, so the final ID will be something like "modID:MyCard"
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        ...
        for(AbstractCard c : p.hand.group){
            if(c instanceof DoubleStrike && c != this){
                c.setCostForTurn(0);
            }
        }
    }
