...
public class UnendingTorment extends BaseCard {
    public static final String ID = makeID("Unending Torment"); //makeID adds the mod ID, so the final ID will be something like "modID:MyCard"
    ...
    public UnendingTorment() {
        super(ID, info); //Pass the required information to the BaseCard constructor.
    }

    @Override
    public void use(AbstractPlayer p, AbstractMonster m) {
        addToBot(new ApplyPowerAction(p, p, new UnendingTormentPower(1)));
    }
    public void upgrade() {
        if (!this.upgraded) {
            upgradeName();
