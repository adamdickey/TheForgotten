...
import forgottenmod.powers.AmnesiaPower;
import forgottenmod.util.CardStats;
import theforgotten.TheForgotten;

public class Amnesia extends BaseCard {
    public static final String ID = makeID("Amnesia"); //makeID adds the mod ID, so the final ID will be something like "modID:MyCard"
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        addToBot(new ApplyPowerAction(p, p, new AmnesiaPower(1), 1));
    }
    public void upgrade() {
        if (!this.upgraded) {
            upgradeName();
