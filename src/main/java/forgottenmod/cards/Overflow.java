...
import forgottenmod.powers.OverflowPower;
import forgottenmod.util.CardStats;
import theforgotten.TheForgotten;

public class Overflow extends BaseCard {
    public static final String ID = makeID("Overflow"); //makeID adds the mod ID, so the final ID will be something like "modID:MyCard"
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        addToBot(new ApplyPowerAction(p, p, new OverflowPower(1), 1));
    }
    public void upgrade() {
        if (!this.upgraded) {
            upgradeName();
