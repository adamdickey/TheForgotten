...
import forgottenmod.powers.OmnipotentPower;
import forgottenmod.util.CardStats;
import theforgotten.TheForgotten;

public class OmnipotentForm extends BaseCard {
    public static final String ID = makeID("Omnipotent Form"); //makeID adds the mod ID, so the final ID will be something like "modID:MyCard"
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        addToBot(new ApplyPowerAction(p, p, new OmnipotentPower(3), 3));
    }
    public void upgrade() {
        if (!this.upgraded) {
