...
import forgottenmod.actions.HandCopyingSkillAction;
import forgottenmod.util.CardStats;
import theforgotten.TheForgotten;

import java.util.ArrayList;

public class HandCopySkill extends BaseCard {
    public static final String ID = makeID("Hand Copying Skill"); //makeID adds the mod ID, so the final ID will be something like "modID:MyCard"
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        addToBot(new HandCopyingSkillAction(cardsInHand, this));
    }
    public void upgrade() {
        if (!this.upgraded) {
            upgradeName();
